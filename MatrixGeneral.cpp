...
struct Matrix {
  vector<vector<T>> dat;
  Matrix(signed N):dat(N,vector<T>(N)){};
  Matrix(const vector<vector<T>>& dat):dat(dat){};
};
template <class T>
Matrix<T> operator*(Matrix<T>& a,Matrix<T>& b) {
  int N = a.dat.size();
	int x,y,z; Matrix<T > res(N);
	rep(x,N) rep(z,N) rep(y,N) {
		res.dat[x][y] += a.dat[x][z] * b.dat[z][y];
	}
	return res;
}
template<class T> Matrix<T> operator^(Matrix<T> a, signed long long k) {
  assert(k>=0);
  int N = a.dat.size();
  Matrix<T> res(N);
