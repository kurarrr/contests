...
int main(){
  ...
  ll a= 0, b = 0;
  REP(j,1,N-1){
    ll i = ll(lower_bound(x.begin(),x.end(),x[j]-D)-x.begin());
    ll k = ll(upper_bound(x.begin(),x.end(),x[j]+D)-x.begin()) - 1;
    a += (k-j) * (j-i);
    dump(i,j,k,x[i],x[j],x[k]);
  }
  REP(i,0,N-2){
    ll k = ll(upper_bound(x.begin(),x.end(),x[i]+D)-x.begin()) - 1;
    dump(i,k);
    b += (k-i) * (k-i-1) / 2;
  }
  dump(a,b);
