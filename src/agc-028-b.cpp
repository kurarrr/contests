...
template<int MOD> istream& operator>>(istream& st, ModInt<MOD> &a) {
  long long sig; st >> sig; a.x = sig < 0 ? sig % MOD + MOD : sig % MOD; return st;
};
...
int main(){
  ...
  vector<mint> a(N);
  rep(i,N) cin >> a[i] ;
  vector<mint> harm(N+1);
  harm[0] = 0;
  harm[1] = 1;
  REP(i,1,N) harm[i+1] = harm[i] + mint(1)/mint(i+1);
  mint ans = 0;
  rep(i,N){
    ans += (harm[i+1]+harm[N-i]-mint(1))*a[i];
  }
  ans *= com.fac[N];
  cout << ans << endl;
  return 0;
